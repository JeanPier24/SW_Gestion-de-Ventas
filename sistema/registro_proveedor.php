...
if($_SESSION['rol'] != 1 and $_SESSION['rol'] != 2)
	...
	if(!empty($_POST))
	{
		...
		if (empty($_POST['proveedor']) || empty($_POST['contacto']) ||empty($_POST['telefono']) || empty($_POST['direccion']))
		{
			...
			if($query_insert){
				$alert='<p class="msqg_save">proveedor guardado correctamente.</p>';
			}
			else{
				$alert='<p class="msqg_error">Error al guardar el proveedor.</p>';
...
<body>
	...
	<section id="container">
		...
		<form action="" method="post">
			...
			<input type="text" name="proveedor" id="nit" placeholder="Nombre del proveedor">
